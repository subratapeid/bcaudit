<?php
// this code is with brutfroce and google recaptcha and enable disable controll.
...
// Flags to enable/disable functionalities
$enableBruteForceSecurity = false; // Set to false to disable brute force security
$enableRecaptcha = false; // Set to false to disable reCAPTCHA
...
function checkLoginAttempts($pdo, $username, $ip) {
    ...
    if ($failedUserAttempts >= $userLimit) {
        blockUser($pdo, $username, $userTimeBound); // Block user for 5 minutes
        return [
            'status' => 'blocked',
            'blocked_type' => 'user',
            'permanent' => false,
    ...
    if ($failedIPAttempts1 >= $ipLimit1) {
        blockIP($pdo, $ip, $ipTimeBound1); // Block IP for 10 minutes
        return [
            'status' => 'blocked',
            'blocked_type' => 'ip',
            'permanent' => false,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($user && password_verify($_POST['password'], $user['password'])) {
        ...
        echo json_encode(['status' => 'success', 'message' => 'Login successful', 'redirect' => 'audit-list.php']);
    } else {
        recordLoginAttempt($pdo, $username, $ip, 'failed');
