                ...
                <form id="bcaForm" autocomplete="off">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="bca_full_name" class="form-control" placeholder="Enter Full Name"
                                required>
                        </div>
    ...
    <script>
        ...
        function renderTable(isDefault) {
            ...
            if (paginatedData.length === 0) {
                ...
                <button 
                        type="button"
                        class="btn btn-danger btn-delete-bca"
                        data-bca-id="${item.bca_id}"
                        title="Delete BCA">
...
document.addEventListener('click', function (e) {
    ...
    if (e.target.closest('.btn-delete-bca')) {

        const button = e.target.closest('.btn-delete-bca');
        const bcaId = button.getAttribute('data-bca-id');

        Swal.fire({
            title: 'Are you sure?',
            text: 'This BCA record will be permanently deleted!',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                fetch('codes/delete_bca.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                    body: JSON.stringify({ bca_id: bcaId })
                })
                .then(res => res.json())
                .then(data => {
